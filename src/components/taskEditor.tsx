...
export default function TaskEditor({editorIndex, setEditorIndex, sortList, setTaskList} : taskEditorProps) {
    ...
    const [professionalValue, setProfessionalValue] = useState(taskData ? taskData.labels.professional : false);
    ...
    const submitButton = () => {
        ...
        if (titleField.current && dateField.current && timeField.current && personalField.current && professionalField.current && urgentField.current && descriptionField.current) {
            arr[editorIndex] = {
    ...
    return (
        <div className={styles.taskEditor}>
            <div className={styles.editorSegment}>
                <h1>Title</h1>
                <input className={styles.editorInput} ref={titleField} value={titleValue} onChange={e => setTitleValue(e.target.value)}/>
            </div>
            <div className={styles.editorSegment}>
                <h1>Due Date</h1>
                <input className={styles.editorInput} ref={dateField} type='date' value={dateValue} onChange={e => setDateValue(e.target.value)}/>
            </div>
            <div className={styles.editorSegment}>
                <h1>Due Time</h1>
                <input className={styles.editorInput} ref={timeField} type='time' value={timeValue} onChange={e => setTimeValue(e.target.value)}/>
            </div>
            <div className={styles.editorSegment}>
                <h1>Labels</h1>
                <div className={styles.labels}>
                    <label>Personal <input type="checkbox" ref={personalField} id="personal" name="personal" checked={personalValue} onChange={e => setPersonalValue(e.target.checked)} /></label>
                    <label>Professional <input type="checkbox" ref={professionalField} id="professional" name="professional" checked={professionalValue} onChange={e => setProfessionalValue(e.target.checked)} /></label>
                    <label>Urgent <input type="checkbox" ref={urgentField} id="urgent" name="urgent" checked={urgentValue} onChange={e => setUrgentValue(e.target.checked)} /></label>
                </div>
            </div>
            <div className={styles.editorSegment}>
                <h1>Description</h1>
                <textarea className={styles.editorInput} ref={descriptionField} value={descriptionValue} onChange={e => setDescriptionValue(e.target.value)} />
            </div>
            <button className={styles.btn} onClick={submitButton}>Submit</button>
            <button className={`${styles.btn} ${styles.discardBtn}`} onClick={discardButton}>Discard</button>
            <button className={styles.exit} onClick ={() => setEditorIndex(null)}>x</button>
        </div>
    )
}