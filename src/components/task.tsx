...
export default function Task({index, setEditorIndex, sortList, setTaskList} : taskProps) {
        ...
        return (
            <div className={styles.taskStatus}>
                <svg className={styles.taskStatusBubble} viewBox='0 0 50 50' onClick={toggleStatus}>
                    ...
                    {taskData.completed && <polyline points="15,25 25,35 40,15" fill="none" stroke="green" strokeWidth="8px" strokeLinecap="round" />}
                </svg>
            </div>
        )
    }
    const TaskLabels = () => {
        return (
            <div className={styles.taskLabels}>
                {taskData.labels.personal && <div className={`${styles.taskLabel} ${styles.personal}`}>Personal</div>}
                {taskData.labels.professional && <div className={`${styles.taskLabel} ${styles.professional}`}>Professional</div>}
                {taskData.labels.urgent && <div className={`${styles.taskLabel} ${styles.urgent}`}>Urgent</div>}
            </div>
        )
    }
    ...
    return (
        <div className={styles.task}>
            <TaskStatus />
            <div className={`${styles.taskEdit} ${styles.taskField}`} onClick={() => setEditorIndex(index)}>Edit</div>
            <div className={`${styles.taskName} ${styles.taskField}`}>{taskData.title}</div>
            <div className={`${styles.taskDueDate} ${styles.taskField}`}>{taskData.date} {taskData.time}</div>
